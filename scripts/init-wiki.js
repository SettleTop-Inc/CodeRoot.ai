...
function parseArgs() {
  ...
  const opts = { remote: 'origin', docs: null, temp: null, dryRun: false };
...
async function main() {
  ...
  if (opts.dryRun) {
    console.log('DRY RUN: would clone wiki URL:', wikiUrl || '<unknown remote URL>');
    console.log('DRY RUN: would copy files from', docsDir, 'to temp wiki repo at', tempBase);
    const samples = await fsp.readdir(docsDir);
    console.log('DRY RUN: files to copy:', samples.join(', '));
  ...
  try {
    ...
    console.log('Cloning wiki repo:', wikiUrl, 'to', tempBase);
    exec(`git clone "${wikiUrl}" "${tempBase}"`);

    console.log('Copying files...');
    await copyRecursive(docsDir, tempBase);
    ...
    exec(`git -C "${tempBase}" commit -m "chore(docs): update wiki pages from docs/wiki"`);
    exec(`git -C "${tempBase}" push`);
